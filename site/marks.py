"""
Athletic marks — generated identity, not colored squares.

Every school gets a designed mark in the familiar high-school forms: shield,
roundel, slanted banner, hex badge. The form, and a small mascot glyph where
the nickname maps to one, are picked deterministically from the school's name
hash; the colors are the school's own. A mark is inline SVG so it costs no
asset pipeline, scales crisply, and keeps the school's fixed colors in every
site scheme.

The glyphs are deliberately simple filled silhouettes — a pennant-and-plywood
booster-club vocabulary, not vector illustration. A mascot without a mapped
glyph gets a pure letterform mark, which half of real high schools use anyway.

Conferences get simpler emblems: a ring or a bar-monogram in institutional
colors, the way leagues actually brand — quieter than their members.
"""
